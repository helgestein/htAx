...
hbuttonImportXRDOnly = uicontrol(fReadInFig, 'Style', 'pushbutton', ...
    'String', 'Begin analysis - XRD', 'Units', 'Normalized', ...
    'Position', [(leftColOffset + colSpacing * 2) ...
    (topRowOffset - 4 * textSpacingVert) ...
    buttonWidth * 2 buttonHeight], ...
    'Callback', {@buttonImportXRDCallback});
...
%% callback functions
    ...
    function buttonImportXRDCallback(hbuttonImportXRD, eventdata, handles)
        if XRDFolder ~= 0
            if EDXFile ~= 0
                if saveFile ~= 0
                    close(fReadInFig);
